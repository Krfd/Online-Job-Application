...
$id = $_GET['updateid'];
$sql = "SELECT * FROM workingstudent WHERE id = $id";
...
if($check) {
    while($row = mysqli_fetch_array($check)) {
        ...
        if(isset($_POST['update'])) {
            ...
            $update = ("UPDATE workingstudent SET lastname = '$lastname', firstname = '$firstname', phone = '$contact', email = '$email', address = '$address' WHERE id = $id");
            $set = mysqli_query($conn, $update);

            if($set) {
                ?>
                <script>
                    alert("UIWS application updated successfully!");
                ...
                ?>
                <script>
                    alert("UIWS application invalid update. Please try again!");
                    window.location.href = "admin_page.php";
                </script>
                <?php
            }
...
<body>
    <div class="container my-5">
        <h2>Working Student Application</h2>

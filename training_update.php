...
if($check) {
    while($row = mysqli_fetch_array($check)) {
        ...
        if(isset($_POST['update'])) {
            ...
            $update = ("UPDATE training SET first_name = '$firstname', last_name = '$lastname', middle_name = '$middlename', age = '$age', bday = '$birthday', phone = '$contact',  address = '$address', email = '$email' WHERE id = $id");
            $set = mysqli_query($conn, $update);

            if($set) {
                ?>
                <script>
                    alert("Training application updated successfully!");
                ...
                ?>
                <script>
                    alert("Training application invalid update. Please try again!");
                    window.location.href = "admin_page.php";
                </script>
                <?php
            }
...
<body>
    <div class="container my-5">
        <h2>Update Training Application</h2>
        <form method="post">
            <fieldset>
                <div class="row">
                    <div class="col">
